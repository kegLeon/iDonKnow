...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speedInMS := meanSpeed(action, duration) * kmhInMsec
	pow := math.Pow(speedInMS, 2)
	heightM := height / cmInM
	walkCalories := ((walkingCaloriesWeightMultiplier*weight + (pow/heightM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	swimmCalories := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimmCalories
}
